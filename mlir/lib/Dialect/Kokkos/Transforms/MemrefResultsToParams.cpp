...
struct MemrefResultsToParamsPass
  ...
  void rewriteFunction(IRRewriter& rewriter, MLIRContext* ctx, func::FuncOp oldFunc) {
    ...
    if(numReturns != 1) {
      llvm::outs() << "Function does not have exactly one return statement\n";
      signalPassFailure();
    }
    if(!oldBody.hasOneBlock()) {
      llvm::outs() << "Function's body region does not have exactly one block\n";
      signalPassFailure();
    }
      ...
      for(auto rv : ret->getOperands()) {
        if(isa<MemRefType>(rv.getType())) {
          ...
          if(auto alloc = dyn_cast<memref::AllocOp>(rvParent.getDefiningOp())) {
            ...
            llvm::outs() << "Memref-typed return value was generated by op other than alloc or shallow copy/cast\n";
            signalPassFailure();
          }
        }
        i++;
