...
#define GEN_PASS_DEF_LINALGGENERICREORDERINGPASS
#include "Transform/Kernel/KernelPasses.h.inc"

struct LinalgGenericReordering
    : impl::LinalgGenericReorderingPassBase<LinalgGenericReordering> {
...
std::unique_ptr<Pass> createLinalgGenericReorderingPass() {
  return std::make_unique<LinalgGenericReordering>();
}
