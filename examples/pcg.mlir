...
module {
  ...
  func.func @pcg(%A: tensor<?x?xf64, #sparse>, %b: tensor<?xf64>, %dinv: tensor<?xf64>, %tol: f64, %maxiter: index) -> (tensor<?xf64>, index, f64) {
    ...
    %x, %p, %z, %r, %final_relres, %rz, %iters = scf.while (%xiter = %x0, %piter = %p0, %ziter = %z0, %riter = %r0, %rziter = %f0, %i = %c0) : (tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, f64, index) -> (tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, f64, index)
    {
      ...
      // Update x and r
      %xnext = func.call @axpby(%f1, %xiter, %alpha, %pnext, %xiter) : (f64, tensor<?xf64>, f64, tensor<?xf64>, tensor<?xf64>) -> tensor<?xf64>
      %rnext = func.call @axpby(%f1, %riter, %malpha, %Apnext, %riter) : (f64, tensor<?xf64>, f64, tensor<?xf64>, tensor<?xf64>) -> tensor<?xf64>
      ...
      %done = arith.cmpf ole, %relres, %tol : f64
      %timeout = arith.cmpi eq, %i, %maxiter : index
      %break = arith.ori %done, %timeout : i1
      %inext = arith.addi %i, %c1 : index
      scf.condition(%break) %xnext, %piter, %ziter, %rnext, %relres, %rz, %inext : tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, f64, f64, index
    } do {
    ^bb0(%xiter : tensor<?xf64>, %piter : tensor<?xf64>, %ziter : tensor<?xf64>, %riter : tensor<?xf64>, %unused : f64, %rziter : f64, %i : index):
      ...
      scf.yield %xiter, %pnext, %riter, %rr, %rznext, %i : tensor<?xf64>, tensor<?xf64>, tensor<?xf64>, f64, f64, index
    }
