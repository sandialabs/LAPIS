...
void run(const Float2D& out1, const Float2D& out2, const Float2D& in1, const Float2D& in2) {
  ...
  Kokkos::parallel_for(policy,
    KOKKOS_LAMBDA(const TeamMem& t) {
      ...
      int i = t.league_rank();
      auto range = Kokkos::make_pair(i, i+1);
      auto out1_sub = Kokkos::subview(out1, range, Kokkos::ALL());
      auto out2_sub = Kokkos::subview(out2, range, Kokkos::ALL());
      auto in1_sub = Kokkos::subview(in1, range, Kokkos::ALL());
      auto in2_sub = Kokkos::subview(in2, range, Kokkos::ALL());
